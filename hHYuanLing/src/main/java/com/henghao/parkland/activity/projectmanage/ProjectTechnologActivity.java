 ...
 * 项目管理 -- 技术交底
 */
public class ProjectTechnologActivity extends ActivityFragmentSupport implements MyCallBack {


    @InjectView(R.id.lv_projecttechnology)
    ...
    private ArrayList<ProjectTechnologEntity> data;
    ...
    public void initData() {
        ...
        data = new ArrayList<>();
        mAdapter = new ProjectTechnologAdapter(this, data, this);
        ...
        checkBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (checkBox.isChecked()) {
                    itemID.clear();
                    for (ProjectTechnologEntity entity : data) {
        ...
        if (url.endsWith(ProtocolUrl.PROJECT_QUERYTECHNOLOGYMSG)) {
            if (jo instanceof BaseEntity) {
                ...
                if (mData.getStatus() > 0) {
                    ...
                    List<ProjectTechnologEntity> homeData = ToolsJson.parseObjecta(jsonStr, type);
                    String topPath = mData.getPath();//图片URL头部地址
                    for (ProjectTechnologEntity entity : homeData) {
    ...
    private void deleteInfo(final List<DeleteEntity> entity) {
        ...
        Request request = builder.post(requestBody).url(ProtocolUrl.ROOT_URL + ProtocolUrl.DELETE_TECHNOLOGY).build();
        mActivityFragmentView.viewLoading(View.VISIBLE);
